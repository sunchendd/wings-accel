...
```bash
# 1. 编译 wheel（sparsekv 通过固定 Docker 镜像构建）
...
# 启用 sparse_kv 时会额外离线安装 vsparse wheel
python3 install.py --features '{"vllm": {"version": "0.17.0", "features": ["sparse_kv"]}}'
...
| vllm | 0.17.0 | sparse_kv | 启用 BMSA sparse KV cache，安装时需要 `vsparse-*.whl` |
...
| 目标 | 说明 |
|---|---|
| `make build` | 编译交付件；其中 sparsekv 使用固定 Docker 镜像构建 |
| `make install` | 编译 + 安装（默认：vllm adaptive_draft_model） |
| `make test` | 运行 pytest |
