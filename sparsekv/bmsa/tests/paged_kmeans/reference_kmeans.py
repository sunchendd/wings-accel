...
def segment_k_means(
    ...
    centroid_indices = torch.arange(num_centroids, dtype=torch.float32, device=key.device) * (
                num_tokens / num_centroids)
    ...
    for _ in range(num_iters - 1):
        centroids = _triton_k_means_train(data, centroids, max_idx=max_idx, normalize_centroids=True,
                                          return_indices=False)

    data = key.reshape((-1, num_tokens, head_dim))
    centroids = centroids.reshape((-1, num_centroids, head_dim))
    centroids, max_idx, max_cluster_size = _triton_k_means_train(data, centroids, normalize_centroids=False,
                                                                 return_indices=True)
