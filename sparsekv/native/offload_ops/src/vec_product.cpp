...
static constexpr uint32_t UNROLL_OFFSET_2 = 2u;  // unroll offset index 2
static constexpr uint32_t UNROLL_OFFSET_3 = 3u;  // unroll offset index 3
// ==================================================================================
// 编译时选择唯一的实现
// ==================================================================================
...
float VecProduct::VectorDotImpl(const float* __restrict q, const float* __restrict k, uint32_t dim)
{
    ...
    for (; i + SCALAR_UNROLL_FACTOR <= dim; i += SCALAR_UNROLL_FACTOR) {
        sum1 += q[i] * k[i];
        sum2 += q[i+1] * k[i+1];
        sum3 += q[i+UNROLL_OFFSET_2] * k[i+UNROLL_OFFSET_2];
        sum4 += q[i+UNROLL_OFFSET_3] * k[i+UNROLL_OFFSET_3];
    }
...
void VecProduct::VectorMeanImpl(const float* __restrict vectors, float* __restrict mean, uint32_t dim, uint32_t numK)
{
    ...
    for (uint32_t k = 1; k < numK; ++k) {
        ...
        for (; i + SCALAR_UNROLL_FACTOR <= dim; i += SCALAR_UNROLL_FACTOR) {
            ...
            mean[i+UNROLL_OFFSET_2] += currentVec[i+UNROLL_OFFSET_2];
            mean[i+UNROLL_OFFSET_3] += currentVec[i+UNROLL_OFFSET_3];
        }
    ...
    for (; i + SCALAR_UNROLL_FACTOR <= dim; i += SCALAR_UNROLL_FACTOR) {
        ...
        mean[i+UNROLL_OFFSET_2] = mean[i+UNROLL_OFFSET_2] * invNumK;
        mean[i+UNROLL_OFFSET_3] = mean[i+UNROLL_OFFSET_3] * invNumK;
    }
