...
static constexpr uint32_t OMP_THREAD_NUM = 16u;
static constexpr int PREFETCH_LOCALITY_HIGH = 3;  // __builtin_prefetch locality: 0=none, 3=all caches
    ...
    for (uint32_t idxHead = 0; idxHead < kHead; ++idxHead) {
        ...
        for (uint32_t idxNorm = 0; idxNorm < numKrepre; ++idxNorm) {
            ...
            if (idxNorm + 1 < numKrepre) {
                __builtin_prefetch(headBase + (idxNorm + 1) * normOffset, 0, PREFETCH_LOCALITY_HIGH);
            }
