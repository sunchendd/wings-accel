...
.PHONY: all build build-lmcache install test check validate list clean dev-setup help
...
## build-lmcache  — 构建 LMCache wheel 矩阵
build-lmcache:
	bash build/build_lmcache.sh
...
##              用法: make check FEATURES='{"vllm":{"version":"0.17.0","features":["ears"]}}'
check:
