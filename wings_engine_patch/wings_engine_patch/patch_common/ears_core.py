"""Shared EARS (Entropy Adaptive Rejection Sampling) core functionality."""
...
LOGGER = logging.getLogger("wings_accel.ears_core")
SUPPORTED_EARS_METHODS = {"mtp", "eagle3", "suffix"}
_EARS_REJECTION_SAMPLER_CLASS = None


def _torch():
    """Lazy import of torch to avoid import-time dependencies."""
    import torch

    return torch


def parse_ears_tolerance(config: dict) -> float:
    """Parse EARS tolerance from config or environment variables.

    Args:
        config: Configuration dictionary (currently unused, for future extension)

    Returns:
        The EARS tolerance value as a float
    """
...
def get_entropy_adaptive_rejection_sampler_class():
    """Get or construct the EntropyAdaptiveRejectionSampler class.

    This function lazily constructs the sampler class on first call and caches it.
    The sampler wraps vLLM's rejection sampler with entropy-adaptive tolerance.

    Returns:
        The EntropyAdaptiveRejectionSampler class
    """
...
def maybe_enable_sampler(runner, base_tolerance: float) -> bool:
    """Maybe enable EARS sampler on a model runner.

    This function checks if EARS should be enabled based on:
    - The tolerance value (must be > 0.0)
    - The speculative method (must be in SUPPORTED_EARS_METHODS)
    - The runner has required fields (rejection_sampler, sampler)
    - The sampler is not already wrapped

    Args:
        runner: The model runner instance to potentially patch
        base_tolerance: The base tolerance value for EARS

    Returns:
        True if the sampler was successfully enabled, False otherwise

    Raises:
        RuntimeError: If an error occurs during sampler construction
    """
    if base_tolerance <= 0.0:
        return False

    spec_config = getattr(runner, "speculative_config", None)
    method = getattr(spec_config, "method", None)
    if method not in SUPPORTED_EARS_METHODS:
        return False
    ...
    try:
        sampler_cls = get_entropy_adaptive_rejection_sampler_class()
        if isinstance(runner.rejection_sampler, sampler_cls):
            return False
        ...
        raise RuntimeError(f"Failed to enable EARS sampler: {e}") from e
