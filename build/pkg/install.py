...
Example:
    ...
    python install.py --features '{"vllm": {"version": "0.19.0", "features": ["ears"]}, "lmcache": {"target": "nvidia-x86"}}'
...
""",
    ...
    try:
        ...
        stderr_logger.error(f"[wings-accel] Error: --features is not valid JSON: {exc}")
        sys.exit(1)
    ...
    engine_feature_configs = {
        ...
        if name != _LMCACHE_COMPONENT_NAME
    }
    has_engine_requests = bool(engine_feature_configs)
    ...
    duplicate_aliases = find_duplicate_engine_aliases(engine_feature_configs)
