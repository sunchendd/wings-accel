...
#   lmcache/<target>/*.whl      (LMCache wheels when enabled)
#   lmcache/<target>/deps/*.whl (offline LMCache dependency wheels when enabled)
set -euo pipefail
...
LMCACHE_OUTPUT_DIR="${OUTPUT_DIR}/lmcache"
LMCACHE_MANIFEST_PATH="${OUTPUT_DIR}/lmcache_manifest.json"
BUILD_LMCACHE="${WINGS_BUILD_LMCACHE:-1}"
...
case "${ARCH}" in
    x86_64)
        LMCACHE_TARGETS_DEFAULT="nvidia-x86"
        ;;
    aarch64)
        LMCACHE_TARGETS_DEFAULT="ascend-arm"
        ;;
    *)
        LMCACHE_TARGETS_DEFAULT=""
        ;;
esac
LMCACHE_TARGETS="${WINGS_LMCACHE_TARGETS:-${LMCACHE_TARGETS_DEFAULT}}"
...
# ---------------------------------------------------------------------------
# 4. Build LMCache wheels for the current host architecture
# ---------------------------------------------------------------------------
if [[ "${BUILD_LMCACHE}" == "1" ]]; then
    [[ -n "${LMCACHE_TARGETS}" ]] || {
        echo "[wings-accel] Error: unsupported ARCH '${ARCH}' for LMCache build" >&2
        exit 1
    }

    echo "[wings-accel] ── Building LMCache targets (${LMCACHE_TARGETS}) for ${ARCH}..."
    bash "${ROOT_DIR}/build/build_lmcache.sh" --outdir "${LMCACHE_OUTPUT_DIR}" --targets "${LMCACHE_TARGETS}"

    mkdir -p "${PKG_DIR}/lmcache"
    cp -R "${LMCACHE_OUTPUT_DIR}/." "${PKG_DIR}/lmcache/"
    cp "${LMCACHE_MANIFEST_PATH}" "${PKG_DIR}/lmcache_manifest.json"
else
    echo "[wings-accel] ── Skipping LMCache wheel build (set WINGS_BUILD_LMCACHE=1 to enable)"
fi
